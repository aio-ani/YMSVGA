...
- (void)next {
    ...
    CGFloat precent = (CGFloat)(self.currentFrame + 1) / (CGFloat)self.videoItem.frames;
    DLog(@"svga load percent is %f",precent);
    if (delegate != nil && [delegate respondsToSelector:@selector(svgaPlayerDidAnimatedToPercentage:)] && self.videoItem.frames > 0) {
        [delegate svgaPlayerDidAnimatedToPercentage:precent];
    }

    __weak typeof(self) weakself = self;
    if (precent >= 1.0) {
        DLog(@"svga load complete");
    }
}

- (void)deleteCache {
    __weak typeof(self) weakself = self;
    weakself.videoItem = nil;
    [weakself clear];
    [weakself clearAudios];
    [weakself clearDynamicObjects];
    [weakself stopAnimation:true];
}
...
//        self.forwardAnimating = YES;
//        if (self.videoItem.FPS == 0) {
//            DLog(@"videoItem FPS could not be 0！");
//            return;
//        }
//        self.displayLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(next)];
//        self.displayLink.frameInterval =  60/self.videoItem.FPS;
...
//        if (sprite.imageKey != nil) {
//            NSString *bitmapKey = [sprite.imageKey stringByDeletingPathExtension];
//            if (weakself.dynamicObjects[bitmapKey] != nil) {
//                bitmap = weakself.dynamicObjects[bitmapKey];
//            }
//            else {
//                bitmap = weakself.videoItem.images[bitmapKey];
//            }
//        }
//        SVGAContentLayer *contentLayer = [sprite requestLayerWithBitmap:bitmap];
//        contentLayer.imageKey = sprite.imageKey;
...
//                CALayer *hostLayer = tempHostLayers[sprite.matteKey];
//                [hostLayer addSublayer:contentLayer];
//                if (![sprite.matteKey isEqualToString:weakself.videoItem.sprites[idx - 1].matteKey]) {
//                    [weakself.drawLayer addSublayer:hostLayer];
//                }
//            } else {
//                [weakself.drawLayer addSublayer:contentLayer];
//            }
//        }
//        if (sprite.imageKey != nil) {
//            if (self.dynamicTexts[sprite.imageKey] != nil) {
//                NSAttributedString *text = weakself.dynamicTexts[sprite.imageKey];
//                CGSize bitmapSize = CGSizeMake(weakself.videoItem.images[sprite.imageKey].size.width * weakself.videoItem.images[sprite.imageKey].scale, weakself.videoItem.images[sprite.imageKey].size.height * weakself.videoItem.images[sprite.imageKey].scale);
//                CGSize size = [text boundingRectWithSize:bitmapSize
//                                                 options:NSStringDrawingUsesLineFragmentOrigin
//                                                 context:NULL].size;
//                CATextLayer *textLayer = [CATextLayer layer];
//                textLayer.contentsScale = [[UIScreen mainScreen] scale];
//                [textLayer setString:weakself.dynamicTexts[sprite.imageKey]];
//                textLayer.frame = CGRectMake(0, 0, size.width, size.height);
//                [contentLayer addSublayer:textLayer];
//                contentLayer.textLayer = textLayer;
//                [contentLayer resetTextLayerProperties:text];
//            }
//            if (weakself.dynamicHiddens[sprite.imageKey] != nil &&
//                [weakself.dynamicHiddens[sprite.imageKey] boolValue] == YES) {
//                contentLayer.dynamicHidden = YES;
//            }
//            if (weakself.dynamicDrawings[sprite.imageKey] != nil) {
//                contentLayer.dynamicDrawingBlock = weakself.dynamicDrawings[sprite.imageKey];
//            }
//        }
//    }];
//    self.contentLayers = tempContentLayers;
...
//    NSMutableArray *audioLayers = [NSMutableArray array];
//    [self.videoItem.audios enumerateObjectsUsingBlock:^(SVGAAudioEntity * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
//        SVGAAudioLayer *audioLayer = [[SVGAAudioLayer alloc] initWithAudioItem:obj videoItem:weakself.videoItem];
//        [audioLayers addObject:audioLayer];
//    }];
//
//    /// 有调整
...
//                __weak typeof(self) weakself = self;
//                [[NSOperationQueue mainQueue] addOperationWithBlock:^{
//                    [weakself setImage:image forKey:aKey];
//                }];
//            }
//        }
//    }] resume];
